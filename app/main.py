...
from app.schemas.calculation import CalculationBase, CalculationResponse, CalculationStatsResponse, CalculationUpdate  # API request/response schemas
...
@app.get("/calculations/stats", response_model=CalculationStatsResponse, tags=["calculations"])
def get_calc_stats(
    current_user = Depends(get_current_active_user),
    db: Session = Depends(get_db)
):
    """
    Return report statistics for the current authenticated user's calculations.
    """
    calcs = db.query(Calculation).filter(Calculation.user_id == current_user.id).all()
    return summarize_calcs(calcs)
