...
class CalculationStatsResponse(BaseModel):
    """
    Schema for reporting summary statistics about a user's calculations.

    This powers the dashboard report section and the
    GET /calculations/stats endpoint.
    """
    total_calculations: int = Field(
        ...,
        ge=0,
        description="Total calculations owned by the current user",
        example=4
    )
    average_inputs_per_calculation: float = Field(
        ...,
        ge=0,
        description="Average number of operands used per calculation",
        example=2.5
    )
    average_result: Optional[float] = Field(
        None,
        description="Average result across calculations with a stored result",
        example=18.75
    )
    operation_counts: Dict[str, int] = Field(
        default_factory=dict,
        description="Calculation counts grouped by operation type",
        example={
            "addition": 2,
            "subtraction": 1,
            "multiplication": 1,
    ...
    latest_calculation_at: Optional[datetime] = Field(
        None,
        description="Creation timestamp for the most recent calculation"
    )
