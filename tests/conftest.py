...
def get_test_db_url() -> str:
    """Use a separate local test DB when one is available."""
    env_db = os.getenv("TEST_DATABASE_URL")
    if env_db:
        return env_db

    url = make_url(settings.DATABASE_URL)
    if url.database == "fastapi_db":
        return url.set(database="fastapi_test_db").render_as_string(hide_password=False)
...
def fastapi_server():
    ...
    srv_env["DATABASE_URL"] = TEST_DB_URL
    srv_env.setdefault("TEST_DATABASE_URL", TEST_DB_URL)
