...
def reg_login(base_url: str, user_data: dict) -> dict:
    """
    ...
    reg_res = requests.post(reg_url, json=user_data)
...
def test_usr_login(base_url: str):
    reg_url = f"{base_url}/auth/register"
    login_url = f"{base_url}/auth/login"
    ...
    # Register user
    reg_res = requests.post(reg_url, json=test_user)
    ...
    for field, expected_type in required_fields.items():
        assert field in login_data, f"missing {field}"
        assert isinstance(login_data[field], expected_type), f"bad type for {field}"
    ...
    expires_at = parse_dt(login_data["expires_at"])
...
def test_calc_add(base_url: str):
    user_data = {
        "first_name": "Calc",
        "last_name": "Adder",
    ...
    token_data = reg_login(base_url, user_data)
...
def test_calc_sub(base_url: str):
    user_data = {
        "first_name": "Calc",
...
def test_calc_mul(base_url: str):
    user_data = {
        "first_name": "Calc",
...
def test_calc_div(base_url: str):
    user_data = {
        "first_name": "Calc",
        "last_name": "Divider",
...
def test_calc_crud(base_url: str):
    user_data = {
        "first_name": "Calc",
        "last_name": "CRUD",
