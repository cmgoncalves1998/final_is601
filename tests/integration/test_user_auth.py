...
def test_dup_usr_reg(db_session):
...
def test_usr_last_login(db_session, fake_user_data):
...
def test_unique_email_usr(db_session):
...
def test_short_pwd_reg(db_session):
...
def test_token_make_verify(db_session, fake_user_data):
...
def test_auth_email(db_session, fake_user_data):
...
def test_missing_pwd_reg(db_session):
